...
def pack(weight_T, scales):
    if weight_T.dtype != torch.half:
        raise ValueError('Only `torch.half` weights are supported.')
    out_f, in_f = weight_T.shape
    tile = 16
    maxq = 2 ** 4 - 1
    s = scales.t()
    w = weight_T.t()
    ...
    s = s.reshape((-1, out_f)).contiguous()
    w = w.reshape((in_f // tile, tile, out_f // tile, tile))
    w = w.permute((0, 2, 1, 3))
    w = w.reshape((in_f // tile, out_f * tile))
    res = w
...
# def pack_tensor(tensor, scales):
#         """Pack a fake-quantized linear layer into this actual Marlin representation.
#         @linear: fake-quantized `torch.nn.Linear` layer to convert (must be of type `torch.half`)
#         @scales: corresponding quantization scales of shape `(infeatures, groups)`
#         """
#         size_k, size_n = tensor.shape
...
#         w = w.reshape((size_k // tile, tile, size_n // tile, tile))
#         w = w.permute((0, 2, 1, 3))
#         w = w.reshape((size_k // tile, size_n * tile))
#         res = w
#         res = res.reshape((-1, _perm.numel()))[:, _perm].reshape(res.shape)
#         q = np.zeros((res.shape[0], res.shape[1] // 8), dtype=np.uint32)
#         res = res.cpu().numpy().astype(np.uint32)
...
#             q |= res[:, i::8] << 4 * i
#         q = torch.from_numpy(q.astype(np.int32)).to(w.device)
#         return (q.to(tensor.device), s.to(tensor.device))
