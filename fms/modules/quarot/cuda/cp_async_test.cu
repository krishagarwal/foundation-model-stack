...
wmma_ker(half* __restrict__ a, half* __restrict__ hads, uint32_t stride_r, uint32_t stride_c) {//, int log_had_size) {
    ...
    //                 "cp.async.cg.shared.global.L2::256B [%2], [%3], 16;\n"
    //                 :: "l"(b_frag_loc), "l"(a_loc), "l"(b_frag_loc + 1), "l"(a_loc + 1)
    //             );
    //         }
...
int main()
{
    ...
    printf("cols: %d\n", cols);
    wmma_ker<chunks_per_warp, warps_per_block, log_had_size><<<dim3(((cols / 16) * (vector_size / 16)) / chunks_per_warp / warps_per_block, 1), dim3(32 * warps_per_block * 2)>>>(dev_ptr, dev_had_16, 1, 16);//, log_had_size);
