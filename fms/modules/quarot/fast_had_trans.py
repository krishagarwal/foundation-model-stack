...
def triton_fast_had_2d(a, had_size=None):
    ...
    block_size_m = min(had_size // 2, THREADS_PER_BLOCK)
    block_size_n = THREADS_PER_BLOCK // block_size_m
    h = 2 * block_size_m
    triton_below_1k_trans[grid](a, a.shape[0], a.shape[1], a.stride(0), a.stride(1), h, BLOCK_SIZE_M=block_size_m, BLOCK_SIZE_N=block_size_n, num_warps=THREADS_PER_BLOCK // 32)
...
if __name__ == "__main__":
    M, N = 128, 45
    a = torch.empty((M, 32 * N), dtype=torch.float16).cuda()
