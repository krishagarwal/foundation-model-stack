...
def _attn_fwd_inner(
    ...
    for start_n in range (block_min, block_max, BLOCK_N):
        ...
        k_s = load_fn(K_S_block_ptr, True, MASK_STEPS and (n_extra_tokens != 0), "zero")
        k_o = load_fn(K_O_block_ptr, True, MASK_STEPS and (n_extra_tokens != 0), "zero")
        if PRE_LOAD_V:
            v = load_fn(V_block_ptr, MASK_STEPS and (n_extra_tokens != 0), PADDED_HEAD, "zero")
            v_s = load_fn(V_S_block_ptr, MASK_STEPS and (n_extra_tokens != 0), True, "zero")
            v_o = load_fn(V_O_block_ptr, MASK_STEPS and (n_extra_tokens != 0), True, "zero")
...
torch.library.define("dequant::attn", "(Tensor q, Tensor k, Tensor v, Tensor k_s, Tensor k_o, Tensor v_s, Tensor v_o, bool is_causal_mask) -> Tensor")

@torch.library.impl("dequant::attn", "cuda")
def forward(q, k, v, k_s, k_o, v_s, v_o, causal):
...
def fake_forward(q, k, v, k_s, k_o, v_s, v_o, causal):
